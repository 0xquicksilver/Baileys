...
export const makeSocket = ({
	...
	const awaitNextMessage = async (sendMsg?: Uint8Array) => {
		...
		const result = promiseTimeout<any>(
			connectTimeoutMs,
			(resolve, reject) => {
	...
	ws.on("CB:iq,type:set,pair-device", async (stanza: BinaryNode) => {
		...
		const noiseKeyB64 = Buffer.from(creds.noiseKey.public).toString(
			"base64"
		);
	...
	if (printQRInTerminal) {
		printQRIfNecessaryListener(ev, logger, qrString);
	}
